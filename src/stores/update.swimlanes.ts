...
export async function updateSwimlaneName(store: BoardsStore, id: string, name: string, boardId: string, order: number): Promise<void> {
  store.error = null;
  try {
    ...
    runInAction(() => {
      store.boards = store.boards.map(board => {
        if (board.id !== boardId) return board; // Only update the relevant board
        ...
        if (swimlaneIndex !== -1) {
          if (board.id && board.createdAtUtc && updatedSwimlaneData?.id && updatedSwimlaneData?.createdAtUtc) {
            ...
            const updatedSwimlaneInstance = new Swimlane({
              ...
              tasks: existingSwimlane.getTasks(), // Preserve existing tasks
            });
...
export async function deleteSwimlane(store: BoardsStore, id: string): Promise<void> {
  store.error = null;
  try {
